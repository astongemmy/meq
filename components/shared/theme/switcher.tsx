...
const ThemeSwitcher = () => {
  const [theme, setTheme] = useState<string>(typeof window !== 'undefined' ? window.localStorage.theme : 'light');
  const themeSwitchContainerRef = useRef<HTMLDivElement | null>(null);
  const themeSwitchRef = useRef<HTMLDivElement | null>(null);
  const [open, setOpen] = useState<boolean>(false);
  const dispatch: AppDispatch = useAppDispatch();
  useClickAway(themeSwitchRef, setOpen);

  const activeThemeIndicator = {
    system: !['light', 'dark'].includes(theme) ? 'bg-theme-primary/10 dark:bg-theme-primary/30' : '',
    light: theme === 'light' ? 'bg-theme-primary/10' : '',
    dark: theme === 'dark' ? 'bg-theme-primary/30' : '',
  };
  ...
  const handleChangeTheme = (e: React.MouseEvent) => {
    const theme = (e.target as HTMLElement).dataset.theme ?? 'light';
  ...
  return (
    <div ref={ themeSwitchContainerRef } className="z-20 md:bg-transparent fixed top-4 md:top-8 lg:top-12 right-16 md:right-24 lg:right-16 xl:right-24 mr-1 flex items-center w-auto">
      ...
      {open && (
        <ul className="absolute z-50 top-16 right-0 bg-white rounded-lg ring-1 ring-theme-primary/10 shadow-lg overflow-hidden w-36 text-lg text-slate-700 dark:text-dark-mode-octonary dark:bg-dark-mode-secondary dark:ring-0 dark:highlight-white/5" aria-labelledby="headlessui-listbox-label-3" aria-orientation="vertical" id="headlessui-listbox-options-5" role="listbox" tabIndex={0} aria-activedescendant="headlessui-listbox-option-21">
          <li
            className={`py-3 px-3 text-sm flex gap-4 items-center cursor-pointer ${activeThemeIndicator.light}`}
          ...
          <li
            className={`py-3 px-3 text-sm flex gap-4 items-center cursor-pointer ${activeThemeIndicator.dark}`}
          ...
          <li
            className={`py-3 px-3 text-sm flex gap-4 items-center cursor-pointer ${activeThemeIndicator.system}`}
